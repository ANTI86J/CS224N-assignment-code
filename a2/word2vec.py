...
def negSamplingLossAndGradient(
    ...
    # neg = outsideVectors[indices]
    # o_val = u_o @ centerWordVec
    # neg_val = neg @ centerWordVec
    # sig_o = sigmoid(o_val)
    # sig_neg = sigmoid(neg_val)
    # loss = -np.log(sig_o) - np.sum(np.log(sig_neg))
    # gradCenterVec =  - u_o * (1 - sig_o) + np.sum(
    #     (1 - sig_neg[:, np.newaxis]) * neg, axis=0
    # )
    # gradOutsideVecs = np.zeros_like(outsideVectors)
    # gradOutsideVecs[outsideWordIdx] = -centerWordVec * (1 - sig_o)
    # for i, neg_idx in enumerate(negSampleWordIndices):
    #     gradOutsideVecs[neg_idx] += centerWordVec * (1 - sig_neg[i])
    ### Please use your implementation of sigmoid in here.
    o_vector = outsideVectors[outsideWordIdx]
    neg_vector = outsideVectors[negSampleWordIndices]  # K * D
    o_val = o_vector @ centerWordVec
    neg_val = neg_vector @ centerWordVec  # K
    sig_o_val = sigmoid(o_val)
    sig_neg_val = sigmoid(neg_val)  # K

    loss = -np.log(sig_o_val) - np.sum(np.log(1 - sig_neg_val))
    gradCenterVec = (sig_o_val - 1) * o_vector + np.sum(
        sig_neg_val[:, np.newaxis] * neg_vector, axis=0
    )

    # gradOutsideVecs = np.zeros((outsideVectors.shape))
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = (sig_o_val - 1) * centerWordVec
    # gradOutsideVecs[
    #     negSampleWordIndices, :] += sig_neg_val[:, np.newaxis] * centerWordVec
    for i, neg_idx in enumerate(negSampleWordIndices):  # 可能有重复，只能用循环哇!!!
        gradOutsideVecs[neg_idx] += sig_neg_val[i] * centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideword in outsideWords:
        ...
        loss_j, grad_center_j, grad_outside_j = word2vecLossAndGradient(centerWordVec, outsidewordIdx, outsideVectors, dataset)
        loss += loss_j
        gradCenterVecs[centerWordIdx] += grad_center_j
        gradOutsideVectors += grad_outside_j
    ### END YOUR CODE
